-- Fix: "column reference target_id is ambiguous" in submit_dating_swipe.
-- Must DROP first because PostgreSQL disallows changing the return type via CREATE OR REPLACE.
-- Renamed output column from target_id -> swipee_id to avoid ambiguity with
-- dating_swipes.target_id inside the ON CONFLICT clause.

drop function if exists public.submit_dating_swipe(uuid, public.dating_swipe_decision);

create function public.submit_dating_swipe(
  p_target_id uuid,
...
begin
  ...
  if not exists (
    ...
    where dp.user_id = v_actor and dp.is_enabled = true
  ) then
  ...
  if not exists (
    select 1 from public.dating_profiles dp
    where dp.user_id = p_target_id and dp.is_enabled = true
  ) then
    return query select p_target_id, false, null::uuid;
    return;
  end if;
