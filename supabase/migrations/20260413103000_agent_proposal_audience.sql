create table if not exists public.agent_proposal_audience (
...
create index if not exists idx_agent_proposal_audience_user
  on public.agent_proposal_audience (user_id, proposal_id);

alter table public.agent_proposal_audience enable row level security;

drop policy if exists "agent_proposal_audience_select_own" on public.agent_proposal_audience;
create policy "agent_proposal_audience_select_own"
on public.agent_proposal_audience
for select
to authenticated
using (auth.uid() = user_id);

insert into public.agent_proposal_audience (proposal_id, user_id)
select p.id, unnest(p.audience_user_ids)
from public.agent_proposals p
where coalesce(array_length(p.audience_user_ids, 1), 0) > 0
...
begin
  ...
  select exists (
    select 1
    from public.agent_proposal_audience
    where proposal_id = p_proposal_id
  )
  into v_has_join_audience;

  if (
      v_has_join_audience
      or coalesce(array_length(v_proposal.audience_user_ids, 1), 0) > 0
    )
    and (
      v_user_id is null
      or (
        ...
        and not (v_user_id = any(v_proposal.audience_user_ids))
      )
    ) then
...
declare
  ...
  v_has_join_audience boolean := false;
begin
  v_actor_id := auth.uid();
  ...
  if (
    ...
    and (
      ...
      and not (v_actor_id = any(v_proposal.audience_user_ids))
    ) then
    raise exception 'not_allowed';
  end if;
